...
class ApiMap extends StatefulWidget {
  const ApiMap({super.key});

  @override
  State<ApiMap> createState() => _ApiMapState();
}

class _ApiMapState extends State<ApiMap> {
  ...
  void initState() {
    _data = const <Model>[
      // Model('India', Color.fromRGBO(255, 215, 0, 1.0),
      //     '       New\nSouth Wales'),
      Model('Africa', Colors.green, 'Togo'),
            ...
            layers: <MapShapeLayer>[
              MapShapeLayer(
                source: MapShapeSource.asset(
                  'assets/world_map.json',
                  shapeDataField: 'name',
                  dataCount: countriesData.length,
                  primaryValueMapper: (int index) => countriesData[index].name,
                  shapeColorValueMapper: (int index) => countriesData[index].color,
                ),
